...
public class DTXFile
{
    ...
    {
        Debug.LogFormat("version:{0},bytes_per_pixel:{1}",version, bytes_per_pixel);
        Read32BitTexture();
        /*
    ...
    /// <summary>
    /// 适用于版本为DTX_VERSION_LT2，像素数据为BPP_32（RGBA32）
    /// </summary>
    private void Read32BitTexture()
    {
        var data = reader.ReadBytes(this.width * this.height * 4);
        Debug.LogFormat("width:{0},height:{1},data:{2}", width, height, data);
        texture = new Texture2D(this.width, this.height, TextureFormat.RGBA32, false);
        texture.LoadRawTextureData(data.ToArray());
        // 原始数据上下颠倒，RB通道颠倒，需要修正
        FlipTexture(ref texture);
        texture.Apply();
        //DebugLogPixel(texture);
    }

    void DebugLogPixel(Texture2D tex)
    {
    ...
    private static void FlipTexture(ref Texture2D texture)
    {
        ...
        for (int y = 0; y < textureHeight; y++)
        {
            ...
            for (int il = yo, ir = yo + textureWidth - 1; il < ir; il++, ir--)
            {
                ...
                Color32 newCol = new Color32(col.b, col.g, col.r, col.a);
                pixels[il] = pixels[ir];
                pixels[ir] = newCol;
            }
        }
